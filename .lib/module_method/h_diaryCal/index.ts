...
const allCal = [...Array(from[0] - to[0]+1)].map((_, i) => {
	...
	$.writeln(to);
	/*@ts-ignore*/
	const allMonthDataStructure = inputData.map((v) => {
		// $.writeln(v);
	});

	// const createMonthDataStructure = (year: number, month: number, holidays: number[]) => {
	// 	const youbi = cal.getYoubi(year, month, 1);
	// 	const monthDays = cal.getMonthDays(year, month);
	// 	const beforeGap: number = youbi;
	// 	const afterGap: number = 35 - (beforeGap + cal.getMonthDays(year, month));
	// 	const monthDaysStructure = monthDays.map((v) => {
	// 		const dayStructure =  new diaryCalenderDayStructure(v);
	// 		if (holidays.indexOf(v) !== -1) dayStructure.holiday = true;
	// };
	// /*@ts-ignore*/
	// [...Array(16)].map((_, i) => {
	// 	const tarYear = i > 11 ? grobalYear + 1 : grobalYear;
	// 	const tarMonth = i > 11 ? i - 11 : i + 1;
	// 	const holidays = holidayData[i];
	// 	createMonthDataStructure(tarYear, tarMonth, holidays);
	// });
};

const hoge = hogeCal(diary.data);
