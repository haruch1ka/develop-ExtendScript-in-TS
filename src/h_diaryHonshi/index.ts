...
const main = () => {
	...
	const c_style_none = characterStyles.getStyle("[なし]");
	...
	for (let i = 0; i < allPageStructure.length; i++) {
		...
		//左肩の数字がある場合はスタイルを適用する
		changeParagraphStyle(pageEntity.sengetsuTextFrame, p_style_left_up);
		if (pageStructure.dayStructureArray[0].isGlay) {
			changeCharacterStyle(pageEntity.sengetsuTextFrame, c_style_glay);
		} else if (pageStructure.dayStructureArray[0].youbiText === "土") {
			changeCharacterStyle(pageEntity.sengetsuTextFrame, c_style_sat);
		} else if (pageStructure.dayStructureArray[0].youbiText === "日" || pageStructure.dayStructureArray[0].holidayText !== "") {
			changeCharacterStyle(pageEntity.sengetsuTextFrame, c_style_sun);
		} else {
			changeCharacterStyle(pageEntity.sengetsuTextFrame, c_style_none);
		}
		//スタイルを条件に応じて適用する
		/*@ts-ignore*/
